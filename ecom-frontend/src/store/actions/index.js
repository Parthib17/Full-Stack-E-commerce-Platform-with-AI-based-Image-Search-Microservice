...
export const fetchProducts = (queryString) => async (dispatch) => {
    try {
        ...
        let endpoint = `/public/products?${queryString}`;
        
        if (keyword) {
            params.delete('keyword');
            const remainingParams = params.toString();
            endpoint = `/public/products/keyword/${encodeURIComponent(keyword)}?${remainingParams}`;
        }
...
export const searchProductsByKeyword = (keyword) => async (dispatch) => {
    try {
        dispatch({ type: "IS_FETCHING" });
        ...
        dispatch({
            type: "FETCH_PRODUCTS",
            payload: data.content,
            // Remove pagination if not needed or keep if API still returns it
...
export const fetchProductsByCategory = (categoryId) => async (dispatch) => {
    try {
        ...
        dispatch({ 
            type: "IS_ERROR",
            payload: error?.response?.data?.message || "Failed to fetch category products",
        });
    }
};
